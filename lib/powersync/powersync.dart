...
Future<String> getDatabasePath() async {
  ...
  return join(dir.path, 'powersync-flutterflow.db');
}
...
Future<void> openDatabase() async {
  ...
  // await loadSupabase(); //assuming FlutterFlow does this in time
...
/// Hack to wait until we have synced data. A future version of the SDK will provide a supported API for this.
Future<bool> waitForInitialSync({timeout = const Duration(seconds: 10)}) async {
...
Future<int> getUploadQueueSize() async {
  final count = (await db.getUploadQueueStats()).count;
  return count;
}

String getLastSynced() {
  final DateFormat formatter = DateFormat('MM/dd/yy HH:mm');
  return db.currentStatus.lastSyncedAt != null
      ? formatter.format(db.currentStatus.lastSyncedAt!)
      : "Not synced yet";
}
